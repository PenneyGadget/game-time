...
var updateNeighbors = function(self) {
  let topNeighborImpact = lookupImpact(self.shape, self.orientation, "bottom");
  let rightNeighborImpact = lookupImpact(self.shape, self.orientation, "left");
  let bottomNeighborImpact = lookupImpact(self.shape, self.orientation, "top");
  let leftNeighborImpact = lookupImpact(self.shape, self.orientation, "right");
  ...
  // self.neighborTop.bottom = topNeighborImpact;
  // self.neighborRight.left = rightNeighborImpact;
  // self.neighborBottom.top = bottomNeighborImpact;
  // self.neighborLeft.right = leftNeighborImpact;
};
...
let lookupImpact = function(shape, orientation, side) {
  ...
  return impacts[shape][orientation][side] || "N";
};

Tadpole.prototype.setShapeAndOrientation = function(constraints) {
  let topMatches = tileOptions["top"][constraints[0]];
  let rightMatches = tileOptions["right"][constraints[1]];
  let bottomMatches = tileOptions["bottom"][constraints[2]];
  let leftMatches = tileOptions["left"][constraints[3]];

  let allOptionsWithDuplicates = topMatches.concat(rightMatches).concat(bottomMatches).concat(leftMatches);

  let objectsMeetingConditions = meetsAllConditions(allOptionsWithDuplicates);
  let shapeAndOrientation = objectsMeetingConditions.sample();
