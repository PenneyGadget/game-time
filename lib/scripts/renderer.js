...
Renderer.prototype.renderTile = function(shape, location, orientation) {
  switch (shape) {
    ...
    case "doubleArc":
      ...
      break;
    case "quadArc":
      this.renderQuadArc(location, orientation)
      break;
  }
};
...
Renderer.prototype.renderSpoke = function(location, orientation) {
  ...
  var img = new Image();
  img.onload = function() {
    dis.context.drawImage(img, startX, startY)
  }

  img.src = "./lib/images/arc.svg";
...
Renderer.prototype.renderDoubleArc = function(location, orientation) {
  ...
  let startY = this.findStartY(location);
  context.beginPath();
  context.arc(startX + this.doubleArcCoordinates[orientation][arc1][adjX], startY + this.doubleArcCoordinates[orientation][arc1][adjY], 20, (Math.PI * this.doubleArcCoordinates[orientation][arc1][pos1]), (Math.PI * this.doubleArcCoordinates[orientation][arc1][pos2]), false);
  context.stroke();
  context.beginPath();
  context.arc(startX + this.doubleArcCoordinates[orientation][arc2][adjX], startY + this.doubleArcCoordinates[orientation][arc2][adjY], 20, (Math.PI * this.doubleArcCoordinates[orientation][arc2][pos1]), (Math.PI * this.doubleArcCoordinates[orientation][arc2][pos2]), false);
  context.stroke();
};

Renderer.prototype.renderQuadArc = function(location, orientation) {
  let startX = this.findStartX(location);
  let startY = this.findStartY(location);

  context.beginPath();
  context.arc(0, 40, 20, (Math.PI * 1.5), (Math.PI * 0.0), false);
  context.lineWidth=1;
  context.stroke();
  context.beginPath();
  context.arc(40, 40, 20, Math.PI, (Math.PI * 1.5), false);
  context.lineWidth=1;
  context.stroke();
  context.beginPath();
  context.arc(0, 0, 20, (Math.PI * 1.5), (Math.PI * 1.0), false);
  context.lineWidth=1;
  context.stroke();
  context.beginPath();
  context.arc(40, 0, 20, (Math.PI * 0.5), (Math.PI * 1.0), false);
  context.lineWidth=1;
  context.stroke();
};

const doubleArcCoordinates = {
  "top": [[40, 0, 0.5, 1.0], [0, 0, 1.5, 1.0]],
  "right": [[40, 40, 1.0, 1.5], [40, 0, 0.5, 1.0]],
  "bottom": [[0, 40, 1.5, 0.0], [40, 40, 1.0, 1.5]],
  "left": [[0, 0, 1.5, 1.0], [0, 40, 1.5, 0.0]]
};
