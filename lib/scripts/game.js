...
Game.prototype.startLevelOne = function() {
  ...
  $('#canvas').click(function (e) {
    ...
    let oldPosition = findPosition[location];
    let newPosition = rotations[oldPosition]
    if(location != null) {
      dis.renderSpoke(location, newPosition);
      findPosition[location] = newPosition;
      dis[location] = newPosition
    };
    if(dis.one == dis[1] && dis.two == dis[2]) {
      alert("You did it! Nice job!");
    }
  });
};

let youWon = function(location, newPosition) {
  // liveArrangement[location] = newPosition;
