...
var renderAllShapes = function(dis, objectIndex) {
  objectIndex.forEach(function(shape) {
    dis.renderer.renderTile(shape.shapeType, shape.tileNumber, shape.currentOrientation);
  });
};
...
Game.prototype.playLevelOne = function() {
  ...
  $('#canvas').click(function (e) {
    ...
    if(youWon(objectIndex)) {
      alert("You did it! Nice job!"); // trigger next level
    }
  });
};

let youWon = function(objectIndex) {
  // objectIndex.every(object => object.currentOrientation === object.winningOrientation);

  objectIndex.every(function (object) {
